...
export default function Register() {
  ...
  function validateUserData() {
    let rules = Joi.object({
      ...
      confirmPassword: Joi.string().required()
        .valid(Joi.ref('password')).messages({'any.only':'password not match confirm password'}),
    });
  ...
  function showAlert(inputName) {
    ...
    if (x.length > 0 && x[0].message.includes("pattern")) {
      return <p style={{color:'red'}}>password should start with capital letter</p>;
    }
  ...
  let submitForm = async (e) => {
    ...
    // // data the respond from Backend
    // // shape of data determind from backend
    // // data.message:success
    // // data.message:display error message
    // console.log(data);
  };
